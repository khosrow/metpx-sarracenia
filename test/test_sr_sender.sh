...
RESULT=0
#Run tests
echo "Running sr_sender test suite:"
echo "-----------------------------"
echo -n "Sending file by using post_document_root instead of directory ... "
test_post_doc
if [ $? -eq 1 ]; then
    RESULT=1
fi
# echo -n "Sending file by parts to destination and reassembling file ... "
# test_parts
# if [ $? -eq 1 ]; then
#         RESULT=1
# fi
echo -n "Testing on_message plugin script ... "
test_plugin_msg
if [ $? -eq 1 ]; then
        RESULT=1
fi
echo -n "Testing do_send plugin script ... "
test_plugin_send
if [ $? -eq 1 ]; then
        RESULT=1
fi
echo -n "Testing on_post plugin script ... "
test_plugin_post
if [ $? -eq 1 ]; then
        RESULT=1
fi
